 ...
 */
export function getFontStyle(style = 'Regular') {
    return { family: TOKENS.fontFamily, style };
}
function resolveBadgeStyle(style, label, icon) {
    if (style === 'chip-icon')
        return 'chip-icon';
    if (style === 'chip')
        return 'chip';
    // Legacy micro + icon, no label → icon-only chip (matches ui.html rolled-out chip)
    if (style === 'micro' && icon && !label.trim())
        return 'chip-icon';
    return 'chip';
}
/**
 ...
 */
export function createBadge(label, style, bgColor, textColor, icon, borderColor, iconSize) {
    const resolvedStyle = resolveBadgeStyle(style, label, icon);
    const border = borderColor !== null && borderColor !== void 0 ? borderColor : bgColor;
    ...
    if (icon) {
        const resolvedIconSize = iconSize !== null && iconSize !== void 0 ? iconSize : TOKENS.badgeIconSize;
