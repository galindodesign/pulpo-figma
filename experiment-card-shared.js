...
export function styleOverviewText(text, role) {
...
export function createOverviewSectionTitle(title) {
...
export function createRolledOutIcon() {
...
export function resolveExperimentDisplayStatus(status, hasRollout) {
    if (hasRollout && status === "completed") {
        return "rolled_out";
    }
    return status;
}
export function createRolledOutBadge() {
    const icon = createRolledOutIcon();
    return createBadge("Rolled out", "chip", TOKENS.badgeRolledOutBg, TOKENS.badgeRolledOutText, icon, TOKENS.badgeRolledOutBorder, TOKENS.badgeRolledOutIconSize);
}
