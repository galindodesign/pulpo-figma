...
Terms used in Pulpo and growth experiment documentation.

| Term | Definition |
| --- | --- |
| **A/B test** | An experiment with a control and one variant (two total). Pulpo labels this automatically when you have two variants. |
| **Control** | The baseline experience you compare against — usually the current production UI. |
| **Experiment step** | The single journey touchpoint where variants are applied. Only this step can hold variants. |
| **Guardrail metric** | A secondary metric that should not get worse while you optimize the primary metric (e.g. support contact rate). |
...
| **Variant** | A treatment you're testing against the control. |
